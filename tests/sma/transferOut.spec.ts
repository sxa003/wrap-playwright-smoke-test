...
test('investmentTransferOut', async ({page,request}) => {
    test.slow();
...
await page.getByRole('button', { name: 'Confirm unconfirmed' }).click();
 ...
 // await page.goto('https://client-dev13.portfolioonline.com.au/expand/TSP16878931/investments/');
 await page.getByTestId('sell-button-b21a83ef-89dd-4a3f-934d-ad3e65f5ae53').click();
 await page.getByTestId('number-input').click();
 await page.getByTestId('number-input').fill('2000');
...
await expect(page.getByTestId('summary-panel-next-container').getByText('Deposit Instruction')).toBeVisible();
